...
import librosa  # For audio processing and time stretching
...
def apply_speed_adjustment(audio_array, sample_rate, speed_factor):
    """
    Adjust the speed of an audio array using librosa's time stretching.
    
    Args:
        audio_array: The audio data as a numpy array
        sample_rate: The sample rate of the audio
        speed_factor: The speed factor (0.5 = half speed, 2.0 = double speed)
        
    Returns:
        numpy.ndarray: The time-stretched audio array
    """
    if speed_factor == 1.0:
        return audio_array
    ...
    try:
        ...
        # Time stretching (rate < 1 makes it slower, rate > 1 makes it faster)
        stretched_audio = librosa.effects.time_stretch(y=audio_array, rate=speed_factor)
        return stretched_audio
    except Exception as e:
        print(f"Warning: Failed to adjust speed: {e}. Using original audio.")
        return audio_array

def tts_workflow(model, tokenizer, input_text, speaker, output_path, chunk_size=150, batch_size=4, speed_factor=1.0):
    ...
    Args:
        ...
        speed_factor (float): Speed adjustment factor (0.5 = half speed, 1.0 = normal, 2.0 = double)
    ...
    try:
        ...
        if len(input_text) <= chunk_size:
            ...
            if speed_factor != 1.0:
                print(f"Adjusting speech speed (factor: {speed_factor})")
                audio_arr = apply_speed_adjustment(audio_arr, 44100, speed_factor)
        ...
        for i, audio_arr in enumerate(audio_arrays):
            ...
            # Apply speed adjustment
            if speed_factor != 1.0:
                print(f"Adjusting speed for chunk {i+1} (factor: {speed_factor})")
                audio_arr = apply_speed_adjustment(audio_arr, 44100, speed_factor)
...
def main():
    ...
    parser.add_argument("--speed", type=float, default=1.0, help="Speech speed factor (0.5 = half speed, 1.0 = normal, 2.0 = double)")
