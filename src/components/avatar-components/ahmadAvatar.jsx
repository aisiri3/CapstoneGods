/**
 * Ahmad Avatar Component (Casual)
 * 
 * Renders Ahmad, the avatar for Malay, Male, Casual settings.
...
export function Ahmad({ lipSyncData, audioUrl, position, rotation, scale, isFiller }) {
  ...
  useEffect(() => {
    fetch('/intros/malay-male-casual-intro.json')
  ...
  useEffect(() => {
    if (isInitialized && introLipSyncData && activeAudio === 'intro') {
      const introAudio = new Audio('/intros/malay-male-casual-intro.wav');
