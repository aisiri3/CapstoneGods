...
class AudioManager {
    ...
    applyVolumeNormalization() {
      ...
      const normalizedBaseVolume = this.normalVolume * normalizationFactor;
      const normalizedLowVolume = this.loweredVolume * normalizationFactor;
    ...
    adjustBackgroundMusicVolume() {
      ...
      if (this.activeAudios.size > 0) {
        ...
        try {
          this.backgroundMusic.volume = this.loweredVolume * normalizationFactor;
        } catch (error) {
    ...
    fadeInVolume(normalizationFactor = 1.0) {
      ...
      try {
        ...
        const targetVolume = this.normalVolume * normalizationFactor;
        const step = 0.02 * normalizationFactor; // Scale step size by normalization factor
